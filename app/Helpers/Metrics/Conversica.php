...
class Conversica
{
    ...
    public function starts()
    {
        ...
        $query = ConversicaModel::whereIn('program', $programs)
            ->where('starts', '>', 0)
            ->get(['program', 'year', 'month', 'starts', 'enroll_term', 'enroll_term_year']);
        return $query;
    }
    ...
    public function merge()
    {
        ...
        foreach ($rows as $row) {
            if ($row['metric'] == 'leads') {
                $row['metric'] = 'Leads';
                if ($row['year'] == 2019) {
                    Actuals2019::insert($row);
                } elseif ($row['year'] == 2018) {
    ...
    {
        $this->clearYear($year);
        $rows = $this->mergeRows($year);
        foreach ($rows as $row) {
            if ($row['metric'] == 'leads') {
                $row['metric'] = 'Leads';
                DB::table('monthly_actuals_' . $year)
                    ->insert($row);
            } elseif ($row['metric'] == 'starts') {
                $row['metric'] = 'StartsLeadmonth';
                Metrics::insert($row);
    ...
    // Clear metrics and actuals tables for a given year
    public function clearYear($year)
    {
    ...
    public function clearConversica()
    {
        Metrics::where('channel', 'Cultivation')
            ->where('initiative', 'CONVERSICA')
            ->delete();
        Actuals2019::where('metric', 'Leads')
            ->where('channel', 'Cultivation')
            ->where('initiative', 'CONVERSICA')
            ->delete();
    ...
    public function mergeRows($year = 0)
    {
        $data = $this->query($year)->get();
    ...
    public function mergeTargets()
    {
        ...
        $monthly = DB::table('conversica_deployment_targets_monthly')
            ->get();
        
        $erase = DB::table('conversica_deployment_targets')->truncate();

        foreach ($monthly as $m) {
            foreach ($months as $index => $month) {
                $mono = $index + 1;
                $insert['month'] = $mono;
                $insert['program'] = $m->program;
                $insert['year'] = $m->year;
                $insert['deployments'] = $m->$month;

                if ($m->$month > 0) {
                    DB::table('conversica_deployment_targets')
                        ->insert($insert);
                }

                echo $m->program . ' - ' . $m->year . ' - ' . $month . ' - ' . $mono . ' = ' . $m->$month . '<br>';
            }
        }
        // return $monthly;
    }
