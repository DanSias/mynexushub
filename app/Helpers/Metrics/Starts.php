...
class Starts
{
    ...
    {
        $programs = Revenue::distinct(['program_rolled'])->get(['program_rolled']);
    ...
    {
        $codes = Revenue::query()->distinct('program_rolled')->pluck('program_rolled')->toArray();
        $this->codes = $codes;
    }
    ...
    public function programSemester($request)
    {
        ...
        $rev = Revenue::whereIn('program', $list)
            ->where('year', $yr)
            ->whereIn('term', $terms)
            ->get($columns);
